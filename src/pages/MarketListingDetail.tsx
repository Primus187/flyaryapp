...
export default function MarketListingDetail() {
  ...
  const details = listingDetailRows(listing, t, i18n.language);

  // plan 8.4: listed listings for everyone get the public link (opens without Flyary), others the app link
  const share = async () => {
    const token = (listing as { share_token?: string }).share_token;
    const isPublic = !!token && canSharePublicly(listing);
    const url = isPublic ? sharedListingUrl(token!) : window.location.href;
    try {
      if (navigator.share) await navigator.share({ title: listing.title, text: isPublic ? t("market.shared.shareText") : undefined, url });
      else { await navigator.clipboard.writeText(url); toast.success(t(isPublic ? "market.shared.linkCopied" : "market.detail.linkCopied")); }
