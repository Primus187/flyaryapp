...
export default function EventForm() {
  ...
  const [form, setForm] = useState({ group_id: "", title: "", description: "", status: "announced", event_date: "", event_time: "09:00", signup_deadline: "", event_type: "", meeting_point: "", instructor: "", launch_helper: "", max_participants: "", chat_link: "" });
  ...
  useEffect(() => {
    ...
    supabase.from("flight_events").select("*").eq("id", loadId).single().then(({ data }) => {
      if (!data) return; const d = new Date(data.event_date);
      setForm({ group_id: data.group_id, title: data.title, description: data.description || "", status: duplicateId ? "announced" : data.status, event_date: duplicateId ? "" : d.toISOString().split("T")[0], event_time: duplicateId ? "09:00" : d.toTimeString().slice(0, 5), signup_deadline: data.signup_deadline ? new Date(data.signup_deadline).toISOString().split("T")[0] : "", event_type: data.event_type || "", meeting_point: data.meeting_point || "", instructor: data.instructor || "", launch_helper: data.launch_helper || "", max_participants: data.max_participants?.toString() || "", chat_link: data.chat_link || "" });
    });
  ...
  const handleSave = async () => {
    ...
    const payload = { group_id: form.group_id, title: form.title, description: form.description || null, status: form.status as any, event_date: eventDate, signup_deadline: form.signup_deadline ? new Date(form.signup_deadline).toISOString() : null, event_type: form.event_type || null, meeting_point: form.meeting_point || null, instructor: form.instructor || null, launch_helper: form.launch_helper || null, max_participants: form.max_participants ? parseInt(form.max_participants) : null, chat_link: form.chat_link || null, created_by: user.id };
    if (isEdit) { const { created_by, ...updatePayload } = payload; const { error } = await supabase.from("flight_events").update(updatePayload).eq("id", id); if (error) toast({ title: t("common.error"), description: error.message, variant: "destructive" }); else { toast({ title: t("events.eventUpdated") }); navigate(`/events/${id}`); } }
    else { const { error } = await supabase.from("flight_events").insert(payload); if (error) toast({ title: t("common.error"), description: error.message, variant: "destructive" }); else { toast({ title: t("events.eventCreated") }); navigate("/events"); } }
    setLoading(false);
  };
