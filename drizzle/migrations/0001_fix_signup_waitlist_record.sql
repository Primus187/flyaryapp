...
BEGIN
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.signed_up IS DISTINCT FROM OLD.signed_up) THEN
    SELECT e.max_participants, e.signup_deadline, e.title
      INTO _max, _deadline, _title
    ...
    ELSE
      ...
      IF FOUND THEN
        ...
        PERFORM public.send_push_notification(_first_wait.user_id, 'Du bist nachgerückt', 'Für ' || COALESCE(_title, 'den Termin') || ' ist ein Platz frei geworden.', '/events/' || NEW.event_id);
      END IF;
    END IF;
  END IF;
  RETURN NEW;
