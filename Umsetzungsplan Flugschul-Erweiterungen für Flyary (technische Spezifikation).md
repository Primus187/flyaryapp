...
- **App-weite Prüfung nach Abschluss von Phase 5 (2026-09-21, auf expliziten Wunsch des Nutzers, „keine Bugs/Mismatches mehr“).** Da echtes Rollen-Testing im Browser in dieser Umgebung nicht möglich ist (kein Playwright-Setup lauffähig, keine Testaccounts, echte Live-Datenbank hinter `.env` – Schreiben von Testdaten dorthin wäre riskant), stattdessen ein systematischer statischer Audit: Production-Build (`vite build`), vollständiger Testlauf, i18n-Vollständigkeitsabgleich de/en/fr, und ein gezielter Abgleich aller `is_group_admin`-geschützten Tabellen gegen ihre UI-Gates (dieselbe Fehlerklasse wie oben). Gefunden und behoben:
  - **i18n:** 8 Übersetzungslücken über drei Locales verteilt, teils seit der ursprünglichen Baseline vorhanden, nicht durch diese Session verursacht – u. a. fehlten in `de.json` fast alle Texte des globalen `NotificationBell.tsx` (Titel, Leerzustand, „mag“/„kommentierte“/Referenztexte), sichtbar für **jeden** Nutzer, nicht nur Flugschul-Rollen. Ebenso fehlten `goals.*`/`follows.*` komplett in `en`/`fr` und `training.grundkurs`/`brevetkurs`/`siku`/`pilot` in `en`/`fr`. Alle acht Lücken geschlossen, alle drei Locales jetzt exakt deckungsgleich (1449 Schlüssel).
  - **Echter Berechtigungsfehler:** `SchoolPeople.tsx` zeigte den „Funktionen zuweisen“-Button und die Funktions-Checkboxen im Bearbeiten-Dialog für jeden Betrachter der Seite (`canManage` war in `SchoolDashboard.tsx` hart auf `true` gesetzt), obwohl `group_member_functions` per RLS **admin-only** ist (nicht `is_group_staff`) – ein Fluglehrer/Schulleiter ohne Admin-Rolle konnte also Funktionen umschalten sehen, aber die Schreibaktion wurde von RLS abgelehnt. Schlimmer: `saveEdit()` prüfte den `error` der `insert`/`delete`-Aufrufe auf `group_member_functions` gar nicht ab, sodass trotz abgelehnter Schreibung ein „Gespeichert“-Toast erschien – ein „false success“, nicht nur ein Berechtigungsproblem. Behoben durch einen neuen, echten `isAdmin`-Prop (aus `group_members.role` der Elternseite abgeleitet statt hartcodiert) für die Funktions-UI, plus explizite Fehlerprüfung in `saveEdit()` als zusätzliche Absicherung. Ausbildungsstufen-Bearbeitung (RLS: `is_group_staff`) blieb unverändert für alle Team-Rollen verfügbar.
